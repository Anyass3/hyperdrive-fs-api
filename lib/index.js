...
class Hyperdrive extends hyperdrive {
    ...
    async put(path, blob, { awaitStats = true, ...opts } = {}) {
        path = path.replace(/\/$/, '');
    ...
    async del(path, { awaitStats = true } = {}) {
    ...
    createFolderWriteStream(path, { awaitStats = true } = {}) {
        const self = this;
    ...
    async #del(path, { resolveStats = true, awaitStats = true }) {
        path = path.replace(/\/$/, '');
